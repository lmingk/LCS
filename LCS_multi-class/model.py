from utils import *
from layers import GraphConvolution, GraphSageConvolution,HighOrderAggregator
import autograd_wl
...
class GraphModel(Net):
    def __init__(self, nfeat, nhid, num_classes, layers, dropout, multi_class,order_list):
        ...
        for id,dim in enumerate( self.dims):
            self.gcs.append(HighOrderAggregator(dim[0],dim[1],dropout,act='relu',order=self.order_list[id],bias='norm'))
