...
class ladies_sampler():
    ...
    def scale_growing(self, adj_lap,original_train_nodes,train_nodes, rate):
        ...
        select_nodes = np.concatenate((inter, index ))[:len(train_nodes)] ####
        ...
        pss = 1 / ps
        pss[np.setdiff1d(train_nodes,support_nodes)] = 0
        adj_matrix = adj_lap[self.nodes,:][:,self.nodes].multiply(pss[self.nodes])
    ...
    def get_test(self, adj_lap,targets):
        ...
        for layer in range(self.n_layer):
            ...
            adj = U[:, neighbors]
            adjs[self.order_index[-layer - 1]] = adj
...
class graphsaint_sampler():
    ...
    def target_nodes_batch(self,batch_nodes):

        previous_nodes = batch_nodes
        sampled_nodes = []
        adjs = [0] * len(self.order_list)
        num_nodes = self.original_matrix.shape[0]
        for d in range(self.n_layer):
            U = self.original_lap_matrix[previous_nodes, :]
            pi = np.array(
                np.sum(self.original_lap_matrix_sq[previous_nodes, :], axis=0))[0]
            p = pi / np.sum(pi)
            s_num = np.min([np.sum(p > 0), int(1 * len(batch_nodes))])
            after_nodes = np.random.choice(
                num_nodes, s_num, p=p, replace=False)
    ...
    def scale_growing(self, adj_lap,original_train_nodes,train_nodes, rate):
        ...
        self.nodes = np.concatenate((self.train_nodes, np.setdiff1d(support_nodes, self.train_nodes)))
        pss = 1 / ps
        pss[np.setdiff1d(train_nodes,support_nodes)] = 1
        adj_matrix = adj_lap[self.nodes,:][:,self.nodes].multiply(pss[self.nodes])
        self.lap_matrix = row_normalize(adj_matrix)
    ...
    def mini_batch(self, seed, batch_num):
        np.random.seed(seed)
        adjs = [0] * len(self.order_list)
        label_nodes = []
        batch_nodes = set()
    ...
    def mini_batch2(self, seed, batch):
        # print(seed,'seeds')
        np.random.seed(seed)
        adjs = [0] * len(self.order_list)
        label_nodes = []
