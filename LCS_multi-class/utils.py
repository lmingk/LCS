...
if __name__ == '__main__':
    ...
    all = 0
    adj_full = sp.load_npz('/Users/apple/Downloads/graphsaintdata/amazon/adj_full.npz').astype(np.bool)
    print(adj_full[0].nonzero())
    num = adj_full.shape[0]
    for i in range(num):

        nei = len(adj_full.getrow(i).indices)

        if nei>1:
            adj_full[i,i] = False
        else:all+=1
    print(adj_full[0].nonzero())
    file = open('/Users/apple/Desktop/adj_full_new.npz','wb')
    sp.save_npz(file,adj_full)
    file.close()
    print(all)
    input()
...
def calculate_sample_grad(net, x, adjs, targets):
    outputs = net.forward(x, adjs)
...
def rw_sample_csr(mat,  n_samples, tp=0.15):
    g_vertices = list(range(mat.shape[0]))

    n_iter = 100 * n_samples
    sample = set()
    if tp == 1.0:
        return node_sample_csr(mat, n_samples)
    ...
    if len(sample) != n_samples:
        print("could not get all sample nodes")





def snowball_sample_csr(mat, n_samples, steps=2):
    g_vertices = list(range(mat.shape[0]))
    sample = set()
    i = 0
    while True:
        i += 1
        x = random.choice(g_vertices)
        sample.add(x)
        todo = [x]
...
def node_sample_csr(mat, n_samples):
    g_vertices = list(range(mat.shape[0]))
...
def edge_sample_csr(mat, n_samples):
    ...
    while len(node_set)<n_samples:
        num = np.random.choice(len(row),1)[0]
        if row[num] == col[num]:continue
        row_num = row[num]
        col_num = col[num]

        node_set.add(row_num)
        node_set.add(col_num)

        if row_num<col_num:edge_set.add((row_num,col_num))
        else:edge_set.add((col_num,row_num))


    data = list(zip(*edge_set))
    rows = []
    cols = []
    ...
    print("edge sampling:", len(rows))
    new_matrix = sp.coo_matrix((np.ones(len(rows)), (rows, cols)), shape=mat.shape).tocsr()


    l = np.sum(new_matrix,axis=0)
    sample = np.where(l)[1]


    if len(sample)!=len(node_set):print('WRONG')

    return sample,new_matrix[:,sample][sample,:]
...
def sampling_csr(mat,n_samples,s_method = ''):

    if s_method == "edge":
        return edge_sample_csr(mat, n_samples)
    elif s_method == "node":
        sample = node_sample_csr(mat, n_samples)

    elif s_method == "rw":
        sample = rw_sample_csr(mat, n_samples)

    elif s_method == "snowball":
        sample = snowball_sample_csr(mat, n_samples)
...
if __name__ == '__main__':
    ...
    role = json.load(open('/Users/apple/Downloads/graphsaintdata/amazon/role.json'))

    class_map = json.load(open('/Users/apple/Downloads/graphsaintdata/amazon/class_map.json'))
    class_map = {int(k): v for k, v in class_map.items()}



    print(isinstance(list(class_map.values())[0], list))
    num_classes = len(list(class_map.values())[0])
    class_arr = np.zeros((len(class_map), num_classes))
    for k, v in class_map.items():
        class_arr[k] = v
    labels = class_arr
