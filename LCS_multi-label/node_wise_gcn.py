...
def graphsage(args, device):
    file = open('./result/' + args.dataset + '_graphsage_sampling.csv', 'w')
    file.write('epoch,cnt,epoch time,train loss,test loss,val f1,test f1\n')
    if args.dataset=='reddit':
        samp_num_list = np.array([5, 5])
    else:
        samp_num_list = np.array([5 for _ in range(sum(args.layer))])

    order_list = args.layer
    args.stop1 = 4
    graphsage_sampler_ = graphsage_sampler(order_list)
    ...
    for phase in range(100):
        ...
        adjs, input_nodes = graphsage_sampler_.target_nodes_batch1(adj_lap,nodes)
        ...
        graphsage_sampler_.scale_growing(adj_lap,train_nodes,labels,target_nodes,args.beta,args)
        ...
        # for mmap mode:
        # features = feats
        features = feats[graphsage_sampler_.nodes]
        labs = labels[graphsage_sampler_.nodes]

        batch_size = get_batch_size(graphsage_sampler_.lap_matrix.shape[0] / len(feats),args.batch_size)
        ...
        while True:
            ...
            t0 = time.time()
            train_data = graphsage_sampler_.get_train_data(args.batch_num, batch_size,samp_num_list)
