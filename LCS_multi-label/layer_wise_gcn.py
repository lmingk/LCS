...
def ladies( args, device):




    file = open('./result/'+args.dataset+'_ladies_sampling.csv','w')
    ...
    if args.dataset == 'pubmed': args.stop1 = 6
    if args.dataset == 'reddit': args.stop1 = 7



    ladies_sampler_ = ladies_sampler(order_list)
    ...
    for phase in range(100):
        ...
        adjs, input_nodes = ladies_sampler_.target_nodes_batch1(adj_lap,nodes)
        ...
        ladies_sampler_.scale_growing(adj_lap,train_nodes,labels,target_nodes,args.beta,args)
        ...
        #for mmap mode:
        #features = feats
        features = feats[ladies_sampler_.nodes]
        labs = labels[ladies_sampler_.nodes]



        batch_size= get_batch_size(ladies_sampler_.lap_matrix.shape[0]/len(feats),args.batch_size)
        ...
        while True:
            ...
            t0= time.time()
            train_data = ladies_sampler_.get_train_data(args.batch_num, batch_size)
